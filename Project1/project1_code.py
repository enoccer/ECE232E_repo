...
def question2_1abc():
    ...
    n = 900
    p = 0.015
    g_900 = ig.Graph.Erdos_Renyi(n=n, p=p, directed=False, loops=False)
    diameter = g_900.diameter()
    ...
    avg_dist, var_dist, final_degrees = simulate_random_walks(g_900, num_trials=2000, max_steps=max_steps)
    ...
    plt.subplot(1,2,1)
    plt.plot(steps, avg_dist, marker='o', linewidth=2)
    plt.xlabel("Step $t$")
    plt.ylabel("Average Distance")
    plt.title(r"$\langle s(t)\rangle$")
    plt.grid(True, alpha=0.4)
    plt.legend()

    plt.subplot(1,2,2)
    plt.plot(steps, var_dist,
             marker='s',
             linewidth=2,
             color='crimson',
             label="N=900")
    ...
    plt.hist(graph_degrees,
             bins=bins,
             density=True,
             alpha=0.5,
             label="Graph Degree Distribution")

    plt.hist(final_degrees,
             bins=bins,
             density=True,
             alpha=0.6,
             label="Random Walk End Nodes")
    ...
    return g_900, avg_dist, var_dist, diameter

def question2_1d(avg900, var900, diameter900):
    
    n = 9000
    p = 0.015

    g_9000 = ig.Graph.Erdos_Renyi(n=n, p=p, directed=False, loops=False)
    ...
    avg9000, var9000, _ = simulate_random_walks(
        g_9000,
        num_trials=1000,
        max_steps=max_steps
    )
